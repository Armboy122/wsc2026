...
#### ข้อกำหนดความปลอดภัยของ Runtime Read-Back Delivery Gate (T6.1 / T6.3)
การอ่านทวนจะถือว่าสำเร็จ (`read_back_completed: true`) และเปิดทางให้รับคำยินยอมจากเสียงได้ ก็ต่อเมื่อผ่านการตรวจสอบเงื่อนไขจริงทั้งหมดในระดับ runtime ดังนี้:
1. **การผูกคู่ Pending Action ID และ Read-Back Generation**: เซิร์ฟเวอร์จับคู่ `pendingActionId` และเลขรุ่น `generation` ในขณะอ่านทวนเสมอ ไม่อนุญาตให้การอ่านทวนของรอบเก่าหรือรายการเดิมที่ล้าสมัยมาเปิดสิทธิ์ให้รายการปัจจุบัน
2. **การส่งมอบแพ็กเก็ตเสียงจริง (Non-Empty Audio Packets)**: ต้องมีการส่งต่อข้อมูลเสียงจริงผ่าน WebSocket ที่มีขนาดมากกว่า 0 ไบต์ (`turn_audio_bytes > 0`) ไม่อนุญาตให้ turn ที่ว่างเปล่าหรือมีแต่สัญญาณ `turn_complete` โดยไร้เสียงจริงถือว่าส่งมอบสำเร็จ
3. **การตรวจสอบข้อความถอดเสียงครบทุกฟิลด์ (All-Field Transcript Verification)**: ข้อความถอดเสียงขาออก (`output_transcription` / text parts) ต้องมีข้อมูลตรงกับทุกฟิลด์ที่จะบันทึกใน `prepared_input` (ห้าม partial read-back หรือข้อความที่ไม่ระบุรายละเอียดสำคัญ)
4. **การรักษาผลโมฆะจากการถูกขัดจังหวะ (Interruption Invalidation Preservation)**: เมื่อผู้ใช้พูดแทรก (`audio.interrupted`) รุ่นการอ่านทวนนั้นจะถูกทำเครื่องหมายเป็นโมฆะทันที และสถานะโมฆะนี้จะคงอยู่ตลอดไปจนกว่าจะมีการเตรียมรายการใหม่ แม้จะมีสัญญาณ `turn_complete` ตามมาใน event เดียวกันหรือหลังจากนั้น
5. **ตัด Unbound Progress Acknowledgement**: ตัดข้อความตอบรับระหว่างดำเนินการที่ไร้ข้อผูกมัดออก เพื่อป้องกันลำดับการส่งมอบเสียงและข้อความสับสน
6. **ขอบเขตและข้อจำกัดการส่งมอบระดับทรานสปอร์ต (Transport Delivery Boundary)**: เซิร์ฟเวอร์ตรวจสอบความถูกต้องได้ถึงระดับการส่งต่อไบต์เสียงและข้อความถอดเสียงลงสู่ WebSocket เท่านั้น โพรโทคอล WebSocket ไม่มีการส่งสัญญาณ ACK ยืนยันว่าลำโพงหรือเบราว์เซอร์ของผู้ใช้ได้เล่นเสียงจบลงแล้วจริง หากเกิดข้อกังขาหรือหลักฐานไม่เพียงพอในจุดใด ระบบต้อง **Fail Closed** เสมอ
