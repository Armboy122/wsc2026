...
class SimulatedVocBackend:
    ...
    def reset(self) -> None:
        """ล้างฉบับร่างที่ยังไม่ได้ส่ง แต่คงเคสที่ส่งแล้วไว้ให้ติดตามได้

        ผู้ใช้ถือ ``vocId`` และ ``trackingKey`` ไว้แล้วหลังส่งเรื่อง และมักกลับมา
        ติดตามในแชตใหม่ภายหลัง การล้างเคสที่ส่งแล้วจะทำให้ติดตามไม่ได้จริง
        จึงล้างเฉพาะฉบับร่างและไม่ย้อนตัวนับ เพื่อไม่ให้ ``vocId`` ซ้ำกับเคสเดิม
        """
